...
export default async function handler(req: any, res: any) {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: {
        parts: [
          ...
          { text: "Transform this image into a high-quality, vibrant, magical masterpiece. If it's a sketch, bring it to life in a Pixar-like 3D style. Keep the main subject recognizable." }
        ]
      }
    });
    ...
    return res.status(500).json({ error: e?.message ?? "Transform failed" });
  }
}
